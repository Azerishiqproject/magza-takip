...
import { ArrowDownRight, ArrowUpRight, CalendarDays, ChevronDown, Download, Layers3, MoreHorizontal, Plus, Search, Store as StoreIcon, TrendingUp, WalletCards, X } from "lucide-react";
...
export function Dashboard() {
  const { stores, categories, transactions, isDemo, addStore, addCategory, addTransaction } = useFinanceData();
  ...
  const [chartDetail, setChartDetail] = useState<{ date: string; type: TransactionType } | null>(null);
  ...
  const chartData = useMemo(() => {
    const groups = new Map<string, { date: string; label: string; gelir: number; gider: number }>();
    storeTransactions.slice().reverse().forEach((item) => {
      const key = item.date;
      const current = groups.get(key) ?? { date: key, label: shortDate(key), gelir: 0, gider: 0 };
      current[item.type === "income" ? "gelir" : "gider"] += item.amount;
      groups.set(key, current);
    });
    return Array.from(groups.values()).slice(-31);
  }, [storeTransactions]);
    ...
    <main className="flex min-h-screen max-w-full overflow-x-hidden">
      ...
      <div className="min-w-0 max-w-full flex-1 overflow-x-hidden">
        ...
        <div className="mx-auto max-w-[1500px] p-4 md:p-6">
          ...
          {view === "categories" && <CategoriesView categories={storeCategories} transactions={transactions} store={activeStore} onAdd={() => setModal("category")} />}
      ...
      <CategoryModal open={modal === "category"} onClose={() => setModal(null)} storeId={activeStore.id} onSubmit={async (data) => { await addCategory(data); setModal(null); toast.success("Kategori oluşturuldu"); }} />
...
function Overview(props: { activeStore: Store; period: Period; setPeriod: (p: Period) => void; customStart: string; customEnd: string; setCustomStart: (v: string) => void; setCustomEnd: (v: string) => void; totalIncome: number; totalExpense: number; profit: number; margin: number; chartData: { date: string; label: string; gelir: number; gider: number }[]; expenseData: { name: string; value: number; color: string }[]; transactions: import("@/lib/types").Transaction[]; categories: Category[]; openTransaction: (type: TransactionType) => void; onChartSelect: (date: string, type: TransactionType) => void }) {
  ...
  return <>
    ...
    <div className="mt-3 grid gap-3 xl:grid-cols-[minmax(0,1.65fr)_minmax(320px,.7fr)]">
      <section className="panel min-h-[360px] rounded-2xl p-4 md:p-5"><SectionTitle title="Gelir ve gider akışı" subtitle="Günün üzerine gelin, gelir veya gider noktasına tıklayın" action={<button className="flex items-center gap-1.5 text-[9px] text-[#84909a]"><Download size={12} />Dışa aktar</button>} /><div className="mt-5 h-[278px]"><ResponsiveContainer width="100%" height="100%"><AreaChart data={props.chartData} margin={{ left: -24, right: 4, top: 10 }}><defs><linearGradient id="income" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#2dd4bf" stopOpacity={.28}/><stop offset="95%" stopColor="#2dd4bf" stopOpacity={0}/></linearGradient><linearGradient id="expense" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#ff7468" stopOpacity={.12}/><stop offset="95%" stopColor="#ff7468" stopOpacity={0}/></linearGradient></defs><CartesianGrid stroke="#252d35" strokeDasharray="3 5" vertical={false}/><XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fill: "#68737d", fontSize: 9 }} minTickGap={25}/><YAxis axisLine={false} tickLine={false} tick={{ fill: "#68737d", fontSize: 9 }} tickFormatter={(v) => `${Math.round(v/1000)}K`}/><Tooltip content={({ active, payload }) => { const point = payload?.[0]?.payload as { date: string; label: string; gelir: number; gider: number } | undefined; if (!active || !point) return null; const dayItems = props.transactions.filter(item => item.date === point.date); return <div className="min-w-[190px] rounded-xl border border-[#2c3740] bg-[#111820] p-3 shadow-2xl"><p className="text-[9px] font-semibold text-white">{point.label} özeti</p><div className="mt-2.5 space-y-2"><div className="flex items-center justify-between gap-5"><span className="text-[9px] text-[#6ddccd]">Gelir · {dayItems.filter(item => item.type === "income").length} kayıt</span><strong className="text-[10px] text-[#6ddccd]">{money(point.gelir, store.currency)}</strong></div><div className="flex items-center justify-between gap-5"><span className="text-[9px] text-[#ff887d]">Gider · {dayItems.filter(item => item.type === "expense").length} kayıt</span><strong className="text-[10px] text-[#ff887d]">{money(point.gider, store.currency)}</strong></div></div><p className="mt-2.5 border-t border-[#29323a] pt-2 text-[8px] text-[#68737d]">Detay için renkli noktaya tıklayın</p></div>; }}/><Area type="monotone" dataKey="gelir" stroke="#2dd4bf" strokeWidth={2} fill="url(#income)" activeDot={(dotProps) => <ClickableDot {...dotProps} color="#2dd4bf" onSelect={(date) => props.onChartSelect(date, "income")} />} /><Area type="monotone" dataKey="gider" stroke="#ff7468" strokeWidth={1.5} fill="url(#expense)" activeDot={(dotProps) => <ClickableDot {...dotProps} color="#ff7468" onSelect={(date) => props.onChartSelect(date, "expense")} />} /></AreaChart></ResponsiveContainer></div></section>
...
function ClickableDot({ cx = 0, cy = 0, payload, color, onSelect }: { cx?: number; cy?: number; payload?: { date?: string }; color: string; onSelect: (date: string) => void }) {
  if (!payload?.date) return <circle cx={cx} cy={cy} r={4} fill={color} />;
  return <circle cx={cx} cy={cy} r={6} fill={color} stroke="#0d1217" strokeWidth={3} className="cursor-pointer outline-none" role="button" tabIndex={0} aria-label={`${payload.date} detaylarını aç`} onClick={() => onSelect(payload.date!)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onSelect(payload.date!); }} />;
}
...
function ChartDetailDrawer({ detail, onClose, transactions, categories, store }: { detail: { date: string; type: TransactionType } | null; onClose: () => void; transactions: import("@/lib/types").Transaction[]; categories: Category[]; store: Store }) {
  if (!detail) return null;
  const items = transactions.filter((item) => item.date === detail.date && item.type === detail.type);
  const total = items.reduce((sum, item) => sum + item.amount, 0);
  const dateLabel = new Intl.DateTimeFormat("tr-TR", { day: "numeric", month: "long", year: "numeric" }).format(new Date(`${detail.date}T12:00:00`));
...
function CategoriesView({ categories, transactions, store, onAdd }: { categories: Category[]; transactions: import("@/lib/types").Transaction[]; store: Store; onAdd: () => void }) { return <PageShell title="Kategoriler" description="Gelir ve giderlerinizi size uygun başlıklarla düzenleyin" action={<button onClick={onAdd} className="flex items-center gap-2 rounded-xl bg-[#2dd4bf] px-3.5 py-2 text-[10px] font-bold text-[#07110f]"><Plus size={14}/>Kategori oluştur</button>}><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{categories.map(category => { const total = transactions.filter(t => t.categoryId === category.id).reduce((s,t) => s+t.amount, 0); return <div key={category.id} className="panel rounded-2xl p-4"><div className="flex items-start justify-between"><span className="grid h-9 w-9 place-items-center rounded-xl" style={{ background: `${category.color}18`, color: category.color }}><Layers3 size={16}/></span><span className={`rounded-full px-2 py-1 text-[8px] font-semibold ${category.type === "income" ? "bg-[#17302b] text-[#65d9ca]" : "bg-[#321f1e] text-[#ff847a]"}`}>{category.type === "income" ? "Gelir" : "Gider"}</span></div><p className="mt-5 text-[11px] font-semibold">{category.name}</p><p className="mt-1 text-[9px] text-[#68737d]">{transactions.filter(t => t.categoryId === category.id).length} işlem</p><p className="mt-4 text-[15px] font-semibold">{money(total, store.currency)}</p></div>})}</div></PageShell>; }
...
function CategoryModal({ open, onClose, storeId, onSubmit }: { open: boolean; onClose: () => void; storeId: string; onSubmit: (data: Omit<Category, "id">) => Promise<void> }) {
  const { submitting, runOnce } = useSubmissionLock();
  return <Modal open={open} onClose={onClose} title="Yeni kategori" description="Raporlarda kullanacağınız özel bir gelir veya gider başlığı ekleyin."><form className="space-y-4" onSubmit={e => { e.preventDefault(); const form = e.currentTarget; void runOnce(async () => { const f = new FormData(form); await onSubmit({ storeId, name: String(f.get("name")), type: String(f.get("type")) as TransactionType, color: String(f.get("color")) }); }); }}><Field label="Kategori adı"><input required disabled={submitting} name="name" className="input" placeholder="Örn. Influencer reklamları"/></Field><div className="grid grid-cols-[1fr_70px] gap-3"><Field label="Kategori tipi"><select disabled={submitting} name="type" className="input"><option value="expense">Gider</option><option value="income">Gelir</option></select></Field><Field label="Renk"><input disabled={submitting} type="color" name="color" defaultValue="#f4b860" className="input h-[39px] p-1.5"/></Field></div><button disabled={submitting} className="w-full rounded-xl bg-[#2dd4bf] py-3 text-[10px] font-bold text-[#07110f] disabled:cursor-not-allowed disabled:opacity-60">{submitting ? "Oluşturuluyor…" : "Kategoriyi oluştur"}</button></form></Modal>;
}
